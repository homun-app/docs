...
> This file is the vision and positioning document. For implementation-oriented documentation of the current codebase, use `docs/services/README.md` and the files under `docs/services/`.
...
| **Homun** | **Rust** | **Agent Skills (open standard)** | **CLI/TG/WA/Discord/Web** | **Yes** | **Ollama native** |

### Key Differentiators

1. **Single Rust binary** — no runtime, no dependencies, instant startup, ~47MB binary (release).
...
5. **27 LLM Providers** — native tool calling, no LiteLLM dependency.
...
### Phase 7: Security & Stability 🔄 IN PROGRESS

#### P0 - Critical (Security & Stability)
- [x] Memory consolidation v2 (LLM classification, dedup, vault integration)
- [x] Shell tool sandboxing (5-layer protection: allowlist, workspace, timeout, etc.)
- [x] Graceful shutdown (ctrl_c(), abort, grace period, DB flush)
- [x] Vault 2FA (TOTP authenticator, 5-min session, recovery codes)
- [x] Exfiltration prevention (T-SEC-02: secret pattern detection + redaction)
...
- [ ] Channel reconnect (auto-reconnect with backoff) — partial
- [ ] Health checks (detailed component status) — partial
- [ ] Retry/backoff (generic for all providers) — partial
...
- [ ] Screenshot/image analysis (vision capabilities)
- [ ] Pre-built binaries (GitHub Releases for macOS/Linux/Windows)
- [ ] Docker image (multi-arch)
- [ ] Homebrew formula (`brew install homun`)
- [ ] Tunnel support (Cloudflare, Tailscale, ngrok)

#### Security (P1)
- [ ] VirusTotal integration (scan skills before install)
- [ ] Skill compatibility analysis (static analysis, risk score)
- [ ] Skill sandbox (isolated execution)
- [ ] Audit logging (log all skill executions)

### Phase 8: Channels Expansion (Q2 2026)
...
#### P1 - User Management
- [ ] User system (users, roles, permissions)
- [ ] Channel auth (map channel IDs to users)
- [ ] Permission enforcement (middleware in agent loop)
...
#### P2 - Marketplace
- [ ] Skill marketplace (verified/community skills)
- [ ] Data marketplace (datasets for skills)

### Phase 10: Workflows & Automations Builder ✅ COMPLETE
- [x] Workflow engine (persistent, multi-step, approval gates, retry, resume-on-boot)
- [x] Trigger system (cron, interval, daily, manual)
- [x] Visual builder (n8n-style SVG canvas, 11 node kinds, drag-to-reorder, guided inspector)
- [x] NLP flow generation (describe automation in natural language → LLM generates flow)
- [x] Unified LLM one-shot engine (`llm_one_shot()`) for all non-conversational LLM calls
...
#### P2 - Documentation
- [ ] Landing page (homun.dev)
- [ ] Documentation site (docs.homun.dev)
- [ ] API reference (OpenAPI spec)
- [ ] Video tutorials (quick start, setup)

### P3 - Polish (Ongoing)
- [ ] Config wizard (guided setup, zero TOML editing)
- [ ] Skill creator UI
- [ ] Bundled skills (4/10 done)
- [ ] Voice transcription
- [ ] Mobile app
- [ ] Monetization
- [ ] Plugin system
...
| Lobster workflows | ✅ Visual Builder + Workflow Engine | Done |
...
| Binary size | 47MB vs 3-8MB | P2 |
| AIEOS identity | ❌ Missing | Future |
| Command allowlist | ✅ Shell sandbox | Done |

### Homun Advantages
| Feature | Homun | OpenClaw | ZeroClaw |
|---------|-------|----------|----------|
| Embeddings locali | ✅ | ❌ | ❌ (API) |
| Web UI | ✅ 19 pages + visual builder | ✅ | ❌ |
| Daily memory files | ✅ | ✅ | ❌ |
| Instruction learning | ✅ | ❌ | ❌ |
| Agent Skills standard | ✅ | ❌ | ❌ |
| WhatsApp nativo Rust | ✅ | Node.js | ❓ |
| Hybrid search (RRF) | ✅ | ❌ | ✅ |
| LLM Providers | 27 | Via LiteLLM | 30+ |
| Vault 2FA | ✅ TOTP | ❓ | ❓ |
