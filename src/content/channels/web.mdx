...
<Callout type="info">
The chat interface uses a **WebSocket** connection for real-time, bidirectional communication. Responses stream token by token as the agent generates them, with no waiting for the full response.
</Callout>
...
The Web UI always uses HTTPS. When accessing remotely, use a reverse proxy (nginx, Caddy, Tailscale) with proper TLS certificates for production use. Only enable `trust_x_forwarded_for` when running behind a **trusted** reverse proxy.
</Callout>
...
<Callout type="info">
The Web UI exposes **50+ REST API endpoints** under `/api/v1/`. These can be used for programmatic access, scripting, or building custom integrations. All API endpoints require authentication (session cookie or API key), except `/api/v1/health`.
</Callout>
...
<Callout type="warning">
Resetting the password this way deletes all user records. After restarting the gateway, you will be prompted to create a new admin password.
</Callout>
