...
Get IMAP and SMTP credentials for your email provider. See the [Provider-Specific Setup](#provider-specific-setup) section below for details.

<Callout type="warning">
Most email providers require **app-specific passwords** instead of your regular password. Gmail, Outlook with MFA, and Fastmail all require app passwords. Using your regular password will fail.
</Callout>

### Step 2: Configure the Account

Add the email account configuration to `~/.homun/config.toml`:
...
Send an email to the configured address to verify everything works.
</Steps>

## How It Works

<Callout type="info">
Homun uses **IMAP IDLE** (RFC 2177) for instant email notifications. Unlike traditional polling, new emails are pushed immediately with no delay. Each email account runs its own IMAP listener task concurrently.
</Callout>
...
<Tabs items={['Gmail', 'Outlook / Microsoft 365', 'Fastmail', 'Self-Hosted']}>
  <Tabs.Tab>
### Gmail
...
<Callout type="warning">
Gmail's "Less secure apps" setting has been removed. You **must** use App Passwords.
</Callout>
  </Tabs.Tab>
  <Tabs.Tab>
...
<Callout type="info">
Make sure your IMAP server supports the IDLE extension. Dovecot supports IDLE by default. Without IDLE support, Homun cannot receive instant push notifications and will fall back to periodic reconnection.
</Callout>
  </Tabs.Tab>
</Tabs>
...
<Callout type="error">
An empty `allow_from` list (`[]`) means **deny all** — no emails are processed. This is the default for safety. You must explicitly configure allowed senders.
</Callout>
...
<Callout type="info">
Passwords can be stored in Homun's encrypted vault instead of in plain text. Set `password = "***ENCRYPTED***"` in config.toml, then store the actual password with `homun vault set email.personal.password "your-app-password"`. The vault uses AES-256-GCM encryption with a master key stored in the OS keychain.
</Callout>

If the user updates the password in the vault at runtime, the email channel picks up the new password on the next IMAP reconnect.
