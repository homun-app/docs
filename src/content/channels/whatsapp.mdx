...
Add the WhatsApp channel to `~/.homun/config.toml`:
...
### Step 3: Pair via QR Code

Open the Web UI and go to **Channels** > **WhatsApp**. Click **Pair** and scan the QR code with your phone (WhatsApp > Settings > Linked Devices > Link a Device).

### Step 4: Start Messaging

Homun responds to incoming messages on your WhatsApp number.
</Steps>

## How It Works

<Callout type="info">
Homun uses the WhatsApp Web multi-device protocol. Your phone does **not** need to be online for Homun to receive messages — they are delivered directly from WhatsApp servers.
</Callout>

When you pair via QR code, your phone registers Homun as a linked device. From that point on:
...
Navigate to `https://localhost:18443` and go to **Channels** > **WhatsApp**.

### Step 3: Scan the QR Code

Click **Pair** — a QR code appears on screen. On your phone: open WhatsApp > **Settings** > **Linked Devices** > **Link a Device**. Scan the QR code.

### Step 4: Wait for Connection

Wait for the connection to establish (usually 5-10 seconds). The Web UI shows "Connected".
</Steps>
...
You need to re-pair when your device has been unlinked. This can happen if you manually removed it from WhatsApp Settings, your phone was offline for more than 14 days, or the session database was deleted or corrupted.
</Callout>
...
<Callout type="info">
WhatsApp's multi-device architecture means your phone does not need to be online for Homun to receive messages. Messages are delivered directly from WhatsApp servers. Linked devices share end-to-end encryption keys with your phone during pairing.
</Callout>
