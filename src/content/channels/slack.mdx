...
<Callout type="info">
**Socket Mode** is the recommended approach. It uses a WebSocket connection from Homun to Slack's servers, meaning no public URL, no ngrok, no webhook endpoint is needed. Events arrive in real-time. The mode is selected automatically based on whether `app_token` is configured.
</Callout>

<Tabs items={['Socket Mode (Recommended)', 'HTTP Polling (Fallback)']}>
  <Tabs.Tab>
**Socket Mode** uses a WebSocket connection from Homun to Slack's servers.

- Requires an app-level token (`xapp-*`)
- No public URL needed
- Real-time latency (sub-second)
- Events delivered via push (WebSocket)
...
```
  ...
  <Tabs.Tab>
**HTTP Polling** is the fallback when no `app_token` is configured.

- Does not require an app-level token
- No public URL needed
- ~3 second latency
- Events delivered via pull (`conversations.history` API)
- Higher API usage — polls every 3 seconds per channel
- Use only for legacy or restricted setups
...
If `mention_required = false`, the bot processes **all messages** in all channels it has access to — which can be noisy in active workspaces. Use this setting only for dedicated bot channels.
</Callout>
...
<Callout type="warning">
Keep both tokens secret. The **bot token** (`xoxb-*`) grants API access to your workspace. The **app token** (`xapp-*`) is used only for Socket Mode connections with limited scope (`connections:write` only). Both can be stored encrypted using Homun's vault.
</Callout>
