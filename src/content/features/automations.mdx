...
<Tabs items={['Cron Jobs', 'Automations', 'Workflows']}>
  <Tabs.Tab>
    **Simple time-based recurring tasks.** Use cron jobs for straightforward scheduled tasks like "check emails every morning" or "send a weekly report on Friday." Standard cron expression syntax, full access to all tools, memory, and skills. Survives gateway restarts. Missed jobs during downtime are skipped (not retroactively run).
  </Tabs.Tab>
  <Tabs.Tab>
    **Event-driven visual flows with conditions and branching.** Use automations for conditional logic like "if an email is urgent, alert me on Telegram; otherwise save a summary." Visual n8n-style flow builder in the Web UI with 11 node types: triggers, conditions, LLM calls, tools, messages. Supports NLP generation: describe what you want in plain language.
  </Tabs.Tab>
  <Tabs.Tab>
    **Multi-step processes with approval gates and retry logic.** Use workflows for business processes that need human approval like "research, draft, review, publish." Persistent state in SQLite (survives restarts), approval gates that pause and wait for your confirmation, retry with exponential backoff (up to 5 retries), resume on boot from last completed step.
  </Tabs.Tab>
</Tabs>
...
The first argument is the cron expression, the second is the natural language instruction.
...
<Callout type="info">
Cron expressions are evaluated in the server's **local timezone**, not UTC. If your server is set to `America/New_York`, `0 9 * * *` runs at 9:00 AM Eastern Time. Keep in mind that daylight saving time transitions may cause jobs to run an hour early or late.
</Callout>
...
<Callout type="info">
Instead of building flows manually, you can describe what you want in plain language and Homun generates the complete automation flow using an LLM. The generated flow appears in the visual builder where you can review, edit, and adjust before activating.
</Callout>
...
If a workflow has been in `waiting_approval` for a long time and nobody noticed, configure the heartbeat to check for pending approvals. Ensure at least one notification channel is active (Telegram, Discord, etc.) so approval requests reach you.
</Callout>
