...
Node.js must be installed on your system (v18 or later). Verify it is available:
...
### Enable browser in configuration
...
### First use downloads the browser
...
Homun automatically injects stealth techniques when launching the browser. Stealth works against most standard bot detection but does not bypass CAPTCHAs, advanced commercial anti-bot services (Cloudflare Turnstile, PerimeterX), or IP-based rate limiting.
</Callout>
...
<Callout type="info">
The browser uses a persistent user data directory, which means cookies, localStorage, IndexedDB, and service workers are preserved across browsing sessions. Log in once (manually or with Homun's help), and subsequent browser tasks can access the authenticated session without re-entering credentials.
</Callout>
...
<Callout type="info">
For debugging browser issues, switch to headed mode (`headless = false`) to see what the browser is doing in real time. Useful during development and debugging, but not suitable for servers without a display. On a remote server, you can use VNC or a remote desktop to view the headed browser.
</Callout>
