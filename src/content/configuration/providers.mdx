...
Never commit API keys to version control. Use the encrypted vault (`homun vault set`) or environment variables for production deployments. See the "Storing Keys Securely" section under each provider.
</Callout>

<Tabs items={['Anthropic', 'OpenAI', 'Ollama', 'OpenRouter', 'DeepSeek', 'Groq']}>
  <Tabs.Tab>
...
OpenRouter is ideal when you want access to models from providers Homun does not natively support (Google Gemini, Cohere, etc.), a single bill for multiple providers, automatic routing to the cheapest available instance of a model, or access to open-weight models (Llama, Mixtral) without running them locally.
...
<Callout type="warning">
Groq's free tier has strict rate limits (30 requests/minute, 14,400 requests/day). If you hit limits frequently, consider using Groq as a fallback rather than primary provider, or upgrade to a paid plan.
</Callout>
...
<Callout type="info">
Configure fallback models so Homun keeps working if your primary provider goes down. The failover chain is tried in order -- the first healthy provider handles the request. Provider health is managed by independent circuit breakers that automatically recover when the provider comes back online.
</Callout>
