...
When exposing Homun remotely, always tighten security settings: enable 2FA, shorten session TTL, enable device approval, and lower rate limits. See the "Security Hardening for Remote Access" section below.
</Callout>

## Access Methods

<Tabs items={['SSH Tunnel', 'Tailscale', 'Cloudflare Tunnel', 'Reverse Proxy', 'Docker']}>
  <Tabs.Tab>
...
The `tls_insecure_skip_verify` is needed because Homun uses a self-signed certificate on its end. Caddy handles the public-facing TLS. Caddy handles WebSocket upgrade automatically.
...
<Callout type="warning">
The `proxy_read_timeout 300s` is critical. LLM responses can take 30+ seconds for complex queries. The default 60s timeout will cause errors. WebSocket headers (`Upgrade`, `Connection`) are required for the chat interface to work.
</Callout>
...
<Callout type="info">
When exposing Homun remotely, tighten these settings regardless of which method you use. For remote access, two-factor authentication is strongly recommended. See [Security > Two-Factor Authentication](/configuration/security#two-factor-authentication-totp).
</Callout>
...
<Callout type="error">
Direct port forwarding is not recommended because Homun's self-signed TLS certificate will trigger browser warnings, your server is directly exposed to the internet, and rate limiting is the only protection against brute-force attacks. If you must use port forwarding, apply all security hardening settings and enable 2FA.
</Callout>
